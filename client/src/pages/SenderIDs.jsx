...
export default function SenderIDs() {
  ...
  const handleRequest = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success('Sender ID application submitted!');
        fetchSenderIds();
        setShowModal(false);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-[#AEB4BC]">Register custom 11-character branded SMS headers</p>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-[rgba(212,175,106,0.15)] font-semibold text-[#AEB4BC] uppercase tracking-wider">
                ...
                <th className="pb-3 px-3 text-right">Date Requested</th>
              </tr>
            </thead>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#2A3038] border border-[rgba(212,175,106,0.3)] rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <h3 className="text-base font-bold text-white">Request Sender ID</h3>
