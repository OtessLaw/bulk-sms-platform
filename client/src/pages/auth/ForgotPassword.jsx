    ...
    <div className="min-h-screen bg-[#1E232B] flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-[#2A3038]/90 backdrop-blur-xl border border-[rgba(212,175,106,0.25)] rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5 relative z-10 mt-12 sm:mt-0 mb-6">
        <div className="text-center space-y-1.5">
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-wider">Reset Your Password</h1>
          <p className="text-xs text-[#AEB4BC]">Enter your email and we will send you a reset link</p>
        </div>

        {success ? (
          <div className="py-6 flex flex-col items-center space-y-3">
            <CheckCircle2 className="w-12 h-12 text-[#D4AF6A] mb-2" />
            <p className="text-sm text-center text-white font-medium">Check your email!</p>
            <p className="text-xs text-center text-[#AEB4BC]">We sent a password reset link to <br/> <span className="text-[#D4AF6A]">{email}</span></p>
          </div>
        ) : (
